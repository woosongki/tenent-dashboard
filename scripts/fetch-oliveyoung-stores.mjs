#!/usr/bin/env node
/**
 * 올리브영 전국 매장 위치 수집 (Kakao Local API)
 * 출력: src/data/oliveyoung.ts
 * 실행: node scripts/fetch-oliveyoung-stores.mjs
 *
 * 약 1,300개 매장 → 시군구 단위까지 세분화 검색
 */
...
const OUT_PATH = path.join(ROOT, "src/data/oliveyoung.ts");
...
const REGIONS = [
  ...
  "강원 춘천","강원 원주","강원 강릉","강원 동해","강원 속초","강원",
  "충북 청주","충북 충주","충북",
  "충남 천안","충남 아산","충남 서산","충남 당진","충남",
  "전북 전주","전북 군산","전북 익산","전북",
  "전남 목포","전남 여수","전남 순천","전남 광양","전남",
  "경북 포항","경북 경주","경북 구미","경북 안동","경북",
  "경남 창원","경남 김해","경남 진주","경남 양산","경남 거제","경남",
  "제주 제주시","제주 서귀포시",
];
...
function isOliveYoung(place) {
  const name = String(place.place_name || "").trim();
  // "올리브영" 정확 또는 "올리브영 OO점" 형식
  return name === "올리브영" || name.startsWith("올리브영 ") || name.startsWith("OLIVE YOUNG ");
}
...
async function main() {
  ...
  for (const region of REGIONS) {
    ...
    for (let page = 1; page <= 3; page++) {
      try {
        const data = await searchKeyword(`${region} 올리브영`, page);
        for (const d of (data.documents || [])) {
          if (!isOliveYoung(d) || seen.has(d.id)) continue;
          seen.set(d.id, {
            id: d.id,
            name: d.place_name,
...
export const OLIVEYOUNG_STORES: ChainStore[] = ${JSON.stringify(stores, null, 2)};
`;
