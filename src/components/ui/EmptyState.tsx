interface Props {
  ...
  /** 커스텀 아이콘 (ReactNode) — 없으면 namedIcon 사용 */
  icon?: React.ReactNode;
  /** 사전 정의된 아이콘 키 */
  namedIcon?: NamedIcon;
  size?: "sm" | "md" | "lg";
  /** 카드/테두리 없이 인라인으로 (테이블 안 등) */
  inline?: boolean;
}
...
export default function EmptyState({
  ...
  const content = (
    <>
      {icon ? (
        <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-50 to-violet-50/40 border border-slate-200/80 text-slate-400">
          {icon}
        </div>
      ) : (
        <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-50 to-violet-50/40 border border-slate-200/80">
      ...
      {description && (
        <p className="mt-1.5 max-w-md text-[13px] text-slate-500 leading-relaxed">{description}</p>
      )}
  ...
  return (
    <div className={`flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white text-center shadow-[0_1px_2px_rgba(15,23,42,.04)] ${SIZE[size]}`}>
      {content}
    </div>
  );
}
