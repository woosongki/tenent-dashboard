...
const NAV = [
  ...
  },
  {
    section: "ANALYTICS",
    items: [
      { href: "/dashboard/sales",     label: "판매분석",    icon: <IconChart /> },
      { href: "/dashboard/drilldown", label: "입점 현황",   icon: <IconBuilding />, badge: "26" },
      { href: "/dashboard/goals",     label: "컨텐츠 POOL", icon: <IconArchive /> },
                ...
                >
                  ...
                  <span
                    className={`h-[15px] w-[15px] shrink-0 [&>svg]:h-full [&>svg]:w-full ${
                      active ? "[&>svg]:stroke-violet-400" : "[&>svg]:stroke-current"
                    }`}
                  >
                    {item.icon}
                  </span>
