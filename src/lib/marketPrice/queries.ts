...
function rowToMarket(r: Record<string, unknown>): MarketPriceRow {
  return {
    ...
    // numeric 컬럼
    deposit_median_num:   (r.deposit_median_num as number | null) ?? null,
    monthly_rent_num:     (r.monthly_rent_num as number | null) ?? null,
    rent_per_pyeong_num:  (r.rent_per_pyeong_num as number | null) ?? null,
    sample_count_num:     (r.sample_count_num as number | null) ?? null,
  };
}
