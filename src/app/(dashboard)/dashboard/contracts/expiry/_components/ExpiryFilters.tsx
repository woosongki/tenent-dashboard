...
interface Props {
  ...
  stores: string[];
  types: string[];
  current: { days: number; store?: string; type?: string };
}

function buildQuery(over: { days?: number; store?: string | null; type?: string | null }, current: Props["current"]): string {
  ...
  if (days !== 60) p.set("days", String(days));
...
export default function ExpiryFilters({ horizons, bandCounts, stores, types, current }: Props) {
  return (
