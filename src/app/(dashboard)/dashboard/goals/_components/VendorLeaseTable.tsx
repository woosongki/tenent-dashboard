...
function StatusBadge({ value }: { value: string | null }) {
  if (!value) return <span className="text-[#0a0a0a]/25">—</span>;
  const m = LEASE_STATUS_META[value];
  if (!m) return <span className="text-xs text-[#0a0a0a]/55 font-medium">{value}</span>;
...
function ScoreBadge({ value }: { value: string | null }) {
  if (!value || value === "미정") return <span className="text-xs text-[#0a0a0a]/30">미정</span>;
  return <span className="text-xs">{value}</span>;
}

function TypeTags({ types }: { types: string[] }) {
  if (!types.length) return <span className="text-[#0a0a0a]/25">—</span>;
  return (
    <div className="flex flex-wrap gap-1">
      {types.slice(0, 3).map((t) => (
        <span key={t} className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-[#0a0a0a]/80">{t}</span>
      ))}
      {types.length > 3 && (
        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-[#0a0a0a]/55 font-medium">+{types.length - 3}</span>
      )}
    </div>
  );
}
...
function KebabMenu({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div ref={ref} className="relative flex justify-center">
      <button type="button" onClick={() => setOpen((v) => !v)} className="flex h-7 w-7 items-center justify-center rounded-lg text-[#0a0a0a]/30 transition-colors hover:bg-slate-100 hover:text-[#0a0a0a]/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400" aria-label="행 메뉴">
      ...
      {open && (
        <div className="absolute right-0 top-8 z-50 w-28 overflow-hidden  border-[2px] border-[#0a0a0a] bg-white py-1 shadow-lg">
          <button onClick={() => { setOpen(false); onEdit(); }} className="flex w-full items-center gap-2 px-3 py-2 text-xs font-medium text-[#0a0a0a]/80 hover:bg-yellow-100 hover:text-violet-700 transition-colors">
          ...
          <button onClick={() => { setOpen(false); onDelete(); }} className="flex w-full items-center gap-2 px-3 py-2 text-xs font-medium text-[#0a0a0a]/55 font-medium hover:bg-rose-50 hover:text-rose-600 transition-colors">
...
// ── Modal ────────────────────────────────────────────────────
const INPUT_CLS = "w-full rounded-lg border-[2px] border-[#0a0a0a] px-3 py-2 text-sm text-[#0a0a0a] placeholder-slate-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 focus-visible:border-transparent";
const LABEL_CLS = "mb-1 block text-xs font-medium text-[#0a0a0a]/65";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto  bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[#0a0a0a] px-6 py-4 sticky top-0 bg-white z-10">
          <h2 className="text-sm font-semibold text-[#0a0a0a]">{isNew ? "라이프스타일 업체 추가" : "라이프스타일 업체 수정"}</h2>
          <button type="button" onClick={onClose} className="text-[#0a0a0a]/30 hover:text-[#0a0a0a]/65 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400 rounded-lg p-1">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                ...
                <button
                  ...
                  className={`rounded-full px-2.5 py-1 text-xs font-medium transition-colors ${
                    selectedTypes.includes(t)
                      ...
                      : "bg-slate-100 text-[#0a0a0a]/80 hover:bg-slate-200"
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is_checked_lease" className="text-xs text-[#0a0a0a]/80">체크 표시</label>
          </div>

          {error && <p className="text-xs text-rose-500">{error}</p>}
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="rounded-lg border-[2px] border-[#0a0a0a] px-4 py-2 text-sm text-[#0a0a0a]/80 hover:bg-slate-50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400">취소</button>
...
function LeaseStatusFunnel({ rows }: { rows: VendorLeaseRow[] }) {
  ...
  return (
    <div className=" border-[2px] border-[#0a0a0a] bg-white px-5 py-4">
      <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-[#0a0a0a]/55 font-medium">입점 파이프라인</p>
        ...
        {counts.map((c) => {
          ...
          return (
            <div key={c.key} className="flex flex-1 flex-col items-center gap-1">
              <span className="text-[11px] font-bold text-[#0a0a0a]">{c.count}</span>
              <div
                className={`w-full rounded-t-sm ${m?.cls.split(" ")[0] ?? "bg-slate-100"}`}
                style={{ height: h }}
              />
              <span className="text-[9px] text-[#0a0a0a]/55 font-medium text-center leading-tight">{c.label}</span>
            </div>
          );
        })}
      </div>
...
export default function VendorLeaseTable({ rows: initialRows }: Props) {
  ...
  const TH = ({ children }: { children: React.ReactNode }) => (
    <th className="px-4 py-3 text-left text-[11px] font-extrabold uppercase tracking-[.12em] text-[#0a0a0a] whitespace-nowrap">{children}</th>
  );
    ...
    <>
      ...
      <div className=" border-[2px] border-[#0a0a0a] bg-white px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="h-8 rounded-lg border-[2px] border-[#0a0a0a] px-3 text-sm text-[#0a0a0a] placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-300 flex-1 min-w-[160px] md:flex-none md:w-44"
          />
          <FilterDrawer activeCount={[fStatus, fType].filter(Boolean).length}>
            <select value={fStatus} onChange={(e) => setFStatus(e.target.value)} className="h-8 rounded-lg border-[2px] border-[#0a0a0a] px-2 text-sm text-[#0a0a0a]/80 focus:outline-none focus:ring-2 focus:ring-violet-300">
              <option value="">전체 상태</option>
              {LEASE_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
            </select>
            <select value={fType} onChange={(e) => setFType(e.target.value)} className="h-8 rounded-lg border-[2px] border-[#0a0a0a] px-2 text-sm text-[#0a0a0a]/80 focus:outline-none focus:ring-2 focus:ring-violet-300">
              <option value="">전체 유형</option>
              {LEASE_TYPE_OPTIONS.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
          </FilterDrawer>
          <span className="ml-auto text-xs text-[#0a0a0a]/55 font-medium hidden sm:inline">{filtered.length} / {rows.length}개 업체</span>
          <button
            type="button"
            onClick={() => setModalRow({})}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center  border-[2px] border-[#0a0a0a] bg-white py-16 text-center">
          <p className="text-sm text-[#0a0a0a]/55 font-medium">{q || fStatus || fType ? "조건에 맞는 업체가 없습니다" : "등록된 라이프스타일 업체가 없습니다"}</p>
          {!q && !fStatus && !fType && (
            <p className="mt-1 text-xs text-[#0a0a0a]/30">노션 동기화 후 데이터가 표시됩니다 (/api/sync/notion)</p>
          )}
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#f8fafc]">
                {filtered.map((row) => (
                  <tr key={row.id} className={`group border-l-[3px] transition-all hover:bg-yellow-100 ${row.is_checked ? "border-l-violet-500 bg-violet-50/30" : "border-l-transparent hover:border-l-violet-500"}`}>
                    <td className="px-4 py-3 text-xs font-semibold text-[#0a0a0a] whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 text-xs text-[#0a0a0a]/80 whitespace-nowrap">{row.keyman ?? <span className="text-[#0a0a0a]/25">—</span>}</td>
                    <td className="px-4 py-3 text-xs text-[#0a0a0a]/65 whitespace-nowrap">{row.contact ?? <span className="text-[#0a0a0a]/25">—</span>}</td>
                    ...
                    <td className="px-4 py-3 text-xs text-[#0a0a0a]/55 font-medium max-w-[160px] truncate">{row.memo ?? <span className="text-[#0a0a0a]/25">—</span>}</td>
