...
function RowModal({ row, onClose }: { row: Partial<VendorFnbRow> | null; onClose: () => void }) {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await upsertVendorFnb(isNew ? null : row!.id!, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto  bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[#0a0a0a] px-6 py-4 sticky top-0 bg-white z-10">
          <h2 className="text-sm font-semibold text-[#0a0a0a]">{isNew ? "F&B 업체 추가" : "F&B 업체 수정"}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select name="score" defaultValue={row?.score ?? ""} className={INPUT_CLS}>
                ...
                {VENDOR_SCORES.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is_checked" className="text-xs text-[#0a0a0a]/80">체크 표시</label>
          </div>
...
// ── Main Table ───────────────────────────────────────────────
interface Props { rows: VendorFnbRow[] }

export default function VendorFnbTable({ rows: initialRows }: Props) {
  const [rows, setRows]     = useState(initialRows);
  const [modalRow, setModalRow] = useState<Partial<VendorFnbRow> | null | false>(false);
    ...
    toast(
      `"${name}" 업체를 삭제하시겠습니까?`,
      {
        action: { label: "삭제", onClick: () => {
          ...
          startTransition(async () => {
            const res = await deleteVendorFnb(id);
    ...
    <>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center  border-[2px] border-[#0a0a0a] bg-white py-16 text-center">
          <p className="text-sm text-[#0a0a0a]/55 font-medium">{q || fStatus || fType ? "조건에 맞는 업체가 없습니다" : "등록된 F&B 업체가 없습니다"}</p>
        </div>
      ) : (
